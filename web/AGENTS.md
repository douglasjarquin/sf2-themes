...
- The project TLD is `.test` (portless's own recommendation), pinned via `PORTLESS_TLD=test` in the task itself rather than a one-time `portless proxy start --tld test`, so the domain is reproducible without an imperative setup step. Deliberately not `.local`: portless's own docs warn it conflicts with mDNS/Bonjour and is the TLD its LAN mode already claims.
